...
public class Question implements Serializable {
	...
	public String toString(){
		return questionNumber+";"+question+";"+Float.toString(betMinimum)+";"+result;
	}

	public Prediction addPrediction(String result, float gains)  {
        Prediction p=new Prediction(result, gains, this);
        predictions.add(p);
        return p;
	}
	...
	public boolean DoesPredictionExists(String result)  {	
		for (Prediction p:this.getPredictions()){
			if (p.getResult().compareTo(result)==0)
				return true;
		}
		return false;
	}
